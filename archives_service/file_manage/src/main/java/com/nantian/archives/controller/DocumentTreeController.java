...
@Api(value = "DocumentTree",tags = {"左侧树管理"})
...
//        QueryWrapper<DocumentTree>   childQuery=new QueryWrapper<>();
//        childQuery.eq("document_class_parent_id",documentTree.getDocumentClassParentId());
//        List<DocumentTree> childList=documentTreeService.list(childQuery);   //获取分类下的子菜单
//        int childSize=childList.size()+1;
...
//                documentTree.setDocumentClassLevel(documentTree.getDocumentClassParentId());
...
//        return   documentTreeService.removeById(id)?  R.ok("数据删除成功"):R.error("数据删除失败");
  ...
  /* @ApiOperation(value="档案分类查看")
    @GetMapping("/selectDocumentList/{fatherId}")
    public  R   selectDocumentList(@PathVariable @ApiParam(value="父级ID",required = true)Integer fatherId){
       QueryWrapper<DocumentTree>  queryWrapper=new QueryWrapper<>();
       queryWrapper.eq("document_class_parent_id",fatherId);
        List<DocumentTree> documentTreeList = documentTreeService.list(queryWrapper);
        return R.ok("成功获取数据列表").data("documentTreeList",documentTreeList);
